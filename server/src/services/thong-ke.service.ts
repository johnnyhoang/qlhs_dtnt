...
export const ThongKeService = {
    ...
    getMealCutoffByClassAndWeek: async (startDate: string, endDate: string) => {
        // Get all active students
        const students = await hocSinhRepository.find({
            where: { trang_thai: TrangThaiHocSinh.DANG_HOC },
    ...
    getMonthlyMealStatistics: async (month: number, year: number) => {
        ...
        const cutRecords = await suatAnRepository.find({
            where: {
                ngay: Between(startDate, endDate),
                bao_cat: true
            }
    ...
    getTransportStatisticsByClass: async (startDate?: string, endDate?: string) => {
        const students = await hocSinhRepository.find({
            where: { trang_thai: TrangThaiHocSinh.DANG_HOC },
            order: { lop: "ASC" }
        });

        const transportRecords = await dinhMucXeRepository.find({
            relations: ["hoc_sinh"]
        });
        ...
        students.forEach(student => {
            ...
            if (transport && transport.khoang_cach > 0) {
                classData.students_with_support++;
                classData.total_distance += Number(transport.khoang_cach);
                classData.total_amount += Number(transport.khoang_cach) * 1000; // Gia tam tinh: khoang cach * 1000
            }
        });
        ...
        return {
            ...
            summary: {
                ...
                total_with_support: transportRecords.filter(t => t.khoang_cach > 0).length,
                total_distance: transportRecords.reduce((sum, t) => sum + Number(t.khoang_cach || 0), 0),
                total_amount: transportRecords.reduce((sum, t) => sum + (Number(t.khoang_cach || 0) * 1000), 0)
            }
        };
    }
};
